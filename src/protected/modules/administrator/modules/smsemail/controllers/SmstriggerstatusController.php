...
class SmstriggerstatusController extends Controller
{
    /**
    ...
    {
        $this->homeUrl = $this->baseUrl .= '/smsemail/smstriggerstatus'; //Backend, I am using for Admin
    ...
    public function actionCreate()
    {
        if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'add')) {
            $model = new SmsTriggerStatus;

            // Uncomment the following line if AJAX validation is needed
            // $this->performAjaxValidation($model);

            if (isset($_POST['SmsTriggerStatus'])) {
                $model->attributes = $_POST['SmsTriggerStatus'];
                if ($model->save())
    ...
    {
        if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'view')) {
            $model = new SmsTriggerStatus('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['SmsTriggerStatus']))
                $model->attributes = $_GET['SmsTriggerStatus'];
     ...
     * @return SmsTriggerStatus the loaded model
     * @throws CHttpException
     */
    public function loadModel($id)
    {
        $model = SmsTriggerStatus::model()->findByPk($id);
        if ($model === null)
     ...
     * @param SmsTriggerStatus $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'sms-trigger-status-form') {
