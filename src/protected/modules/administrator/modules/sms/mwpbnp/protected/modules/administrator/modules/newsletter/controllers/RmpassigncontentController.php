...
class RmpassigncontentController extends Controller
{
	/**
	...
	public $resource_id=25;
        ...
        public function init() {
            $this->homeUrl=$this->baseUrl .= '/rmpassigncontent'; //Backend, I am using for Admin
	...
	public function actionCreate()
	{
            if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'add')) {
                $model=new RmpAssignContent;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['RmpAssignContent']))
		{
			$model->attributes=$_POST['RmpAssignContent'];
			if($model->save())
	...
	{
            if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'view')) {
                $model=new RmpAssignContent('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RmpAssignContent']))
			$model->attributes=$_GET['RmpAssignContent'];
	 ...
	 * @return RmpAssignContent the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=RmpAssignContent::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param RmpAssignContent $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='rmp-assign-content-form')
		{
