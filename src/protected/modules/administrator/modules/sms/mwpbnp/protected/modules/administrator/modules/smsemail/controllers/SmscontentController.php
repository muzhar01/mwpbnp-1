...
class SmscontentController extends Controller
{
    ...
    {
        $this->homeUrl = $this->baseUrl .= '/smsemail/smscontent'; //Backend, I am using for Admin
    ...
    public function actionCreate()
    {
        if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'add')) {
            ...
            if (isset($_POST['SmsContent'])) {
                $model->attributes = $_POST['SmsContent'];
                if ($model->save())
    ...
    {
        if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'view')) {
            $model = new SmsContent('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['SmsContent']))
                $model->attributes = $_GET['SmsContent'];
     ...
     * @return SmsContent the loaded model
     ...
     * @param SmsContent $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'sms-content-form') {
