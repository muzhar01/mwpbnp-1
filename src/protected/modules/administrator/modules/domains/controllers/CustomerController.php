...
class CustomerController extends Controller
{
	...
	public $resource_id=43;
        ...
        public function init() {
            $this->homeUrl=$this->baseUrl .= '/domains/customer'; //Backend, I am using for Admin
	...
	public function actionCreate()
	{
            if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'add')) {
                $model=new MwpCustomer;

		// Uncomment the following line if AJAX validation is needed
		 $this->performAjaxValidation($model);

		if(isset($_POST['MwpCustomer']))
		{
			$model->attributes=$_POST['MwpCustomer'];

            if(!empty($model->profile_image)){
                $rnd = rand(0,9999);
                $uploadedFile=CUploadedFile::getInstance($model,'profile_image');
                $fileName = "{$rnd}-{$uploadedFile}";  // random number + file name
                $uploadedFile->saveAs(Yii::app()->basePath.'/../images/customers/'.$fileName);  // image will uplode to /images/products/
                $model->profile_image = $fileName;
            }
            if($model->save()) {
                Yii::app()->user->setFlash('success', "Customer is saved successfully!");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['MwpCustomer']))
		{
            ...
            if(!empty($_FILES['MwpCustomer']['name']['profile_image'])) {
                $rnd = rand(0,9999);
                $uploadedFile=CUploadedFile::getInstance($model,'profile_image');
                $fileName = "{$rnd}-{$uploadedFile}";  // random number + file name
	...
	{
            if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'view')) {
                $model=new MwpCustomer('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MwpCustomer']))
			$model->attributes=$_GET['MwpCustomer'];
	 ...
	 * @return MwpCustomer the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=MwpCustomer::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param MwpCustomer $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='mwp-customer-form')
		{
