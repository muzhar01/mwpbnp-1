...
class WebpagesController extends Controller
{
	...
	public $resource_id=1;
        ...
        public function init() {
            $this->homeUrl=$this->baseUrl .= '/webpages'; //Backend, I am using for Admin
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Webpages']))
		{
			$model->attributes=$_POST['Webpages'];
			if($model->save())
	...
	{
            if (AdminUser::model()->findByPk(Yii::app()->user->id)->checkAccess($this->resource_id, 'view')) {
                $model=new Webpages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Webpages']))
			$model->attributes=$_GET['Webpages'];
	 ...
	 * @return Webpages the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Webpages::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Webpages $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='webpages-form')
		{
